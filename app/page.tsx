...
function LoginPage({ onLogin }: { onLogin: (role: UserRole) => void }) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (selectedRole === 'qc' && username === 'qc' && password === 'qc123') {
      onLogin('qc');
      return;
    }
    
    if (selectedRole === 'sales' && username === 'sales' && password === 's123') {
      onLogin('sales');
      return;
    }
    
    if (selectedRole === 'maintenance' && username === 'mainten' && password === 'm123') {
      onLogin('maintenance');
      return;
    }
...
function ReportForm({
  ...
  const calculateTotalScore = () => {
    ...
    return { totalScore, maxScore, totalItems, percentage: Math.round(percentage) };
  };
    ...
    if (isQC && formData.status === 'draft') {
      if (isPemeriksaan) {
        ...
        if (emptyItems.length > 0) {
          alert(`Terdapat ${emptyItems.length} item yang belum diisi status.`);
          return;
        }
        ...
        if (invalidItems.length > 0) {
          alert(`Terdapat ${invalidItems.length} item Not Good tanpa temuan.`);
          return;
        }
        ...
        formData.units.forEach((unit: UnitData, idx: number) => {
          ...
          if (emptyItems.length > 0) {
            alert(`Unit ${idx + 1}: Terdapat ${emptyItems.length} item yang belum diisi status.`);
            hasError = true;
          }
        });
        if (hasError) return;
        ...
        {isPemeliharaan && (
          <>
            {formData.units && formData.units.map((unit: UnitData, unitIndex: number) => {
              const unitScore = calculateUnitScore(unit);
                ...
                <div key={unitIndex} className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden mb-8">
                  <div className="bg-blue-700 px-6 py-3 flex items-center justify-between">
                    ...
                    <span className="text-sm text-white bg-blue-600 px-3 py-1 rounded-full">
                      {unit.maintenanceData.length} Item
                    </span>
                  </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'unitNumber', e.target.value)}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'projectCode', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'customerName', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'buildingLocation', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'elevatorType', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={unit.capacity || ''}
                          onChange={(e) => updateUnitData(unitIndex, 'capacity', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={unit.speed || ''}
                          onChange={(e) => updateUnitData(unitIndex, 'speed', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => updateUnitData(unitIndex, 'inspectionDate', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                        />
                      </div>
                      <div>
                        ...
                        <input
                          type="text"
                          value={unit.qcName || ''}
                          onChange={(e) => updateUnitData(unitIndex, 'qcName', e.target.value)}
                          className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 focus:ring-blue-500"
                          disabled={isReadOnly || !isQC || formData.status !== 'draft'}
                        ...
                        {unit.maintenanceData.map((item: any) => {
                          ...
                          return (
                            <tr key={item.id} className="border-b border-gray-100 print:break-inside-avoid">
                                ...
                                {isEditable ? (
                                  <div className="flex flex-col gap-1">
                                    <label className="inline-flex items-center" style={{ fontSize: '12px' }}>
                                      <input type="radio" name={`unit-${unitIndex}-status-${item.id}`} value="Good" checked={item.status === 'Good'} onChange={() => handleMaintenanceStatusChange(unitIndex, item.id, 'Good')} className="w-4 h-4 text-green-600"/>
                                      <span className="ml-1">Good</span>
                                    </label>
                                    <label className="inline-flex items-center" style={{ fontSize: '12px' }}>
                                      <input type="radio" name={`unit-${unitIndex}-status-${item.id}`} value="Not Good" checked={item.status === 'Not Good'} onChange={() => handleMaintenanceStatusChange(unitIndex, item.id, 'Not Good')} className="w-4 h-4 text-red-600"/>
                                      <span className="ml-1">Not Good</span>
                                    </label>
                                    <label className="inline-flex items-center" style={{ fontSize: '12px' }}>
                                      <input type="radio" name={`unit-${unitIndex}-status-${item.id}`} value="N/A" checked={item.status === 'N/A'} onChange={() => handleMaintenanceStatusChange(unitIndex, item.id, 'N/A')} className="w-4 h-4 text-gray-400"/>
                              ...
                              <td className="py-2 px-2 align-top">
                                {isEditable ? (
                                  <textarea value={item.finding} onChange={(e) => handleMaintenanceFindingChange(unitIndex, item.id, e.target.value)} disabled={item.status === 'Good' || item.status === 'N/A'} placeholder={isNotGood ? 'Wajib diisi' : 'Temuan'} className={`w-full px-2 py-1 border rounded-md ${isNotGood && !item.finding ? 'border-red-300 bg-red-50' : 'border-gray-300'} ${item.status === 'Good' ? 'bg-gray-100' : 'bg-white'}`} rows={2} style={{ fontSize: '13px' }}/>
                                ) : (
                              ...
                              <td className="py-2 px-2 align-top">
                                {isEditable && isNotGood ? (
                                  <div>
                                    <input type="file" accept="image/*" onChange={(e) => handleMaintenancePhotoBeforeChange(unitIndex, item.id, e)} style={{ fontSize: '12px' }}/>
                              ...
                              <td className="py-2 px-2 align-top">
                                {isMaintEditable ? (
                                  <div>
                                    <input type="file" accept="image/*" onChange={(e) => handleMaintenancePhotoAfterChange(unitIndex, item.id, e)} style={{ fontSize: '12px' }}/>
                              ...
                              <td className="py-2 px-2 align-top">
                                {isMaintEditable ? (
                                  <textarea value={item.repairNote || ''} onChange={(e) => handleMaintenanceRepairNoteChange(unitIndex, item.id, e.target.value)} placeholder="Catatan perbaikan..." className={`w-full px-2 py-1 border rounded-md ${isNotGood && !item.repairNote ? 'border-red-300 bg-red-50' : 'border-gray-300'}`} rows={2} style={{ fontSize: '13px' }}/>
                                ) : (
                  ...
                  <div className="p-4 bg-gray-50 border-t border-gray-200">
                    <div className="grid grid-cols-2 md:grid-cols-6 gap-3">
                      <div className="bg-white p-3 rounded-lg text-center border border-gray-200">
                        ...
                        <div className="text-lg font-bold text-gray-700">{unitScore.totalItems}</div>
                      </div>
                      <div className="bg-green-50 p-3 rounded-lg text-center border border-green-200">
                        <div className="text-xs text-green-600">Good</div>
                        <div className="text-lg font-bold text-green-700">{unitScore.goodCount}</div>
                      </div>
                      <div className="bg-red-50 p-3 rounded-lg text-center border border-red-200">
                        <div className="text-xs text-red-600">Not Good</div>
                        <div className="text-lg font-bold text-red-700">{unitScore.notGoodCount}</div>
                      </div>
                      <div className="bg-gray-50 p-3 rounded-lg text-center border border-gray-200">
                        <div className="text-xs text-gray-500">N/A</div>
                        <div className="text-lg font-bold text-gray-700">{unitScore.naCount}</div>
                      </div>
                      <div className="bg-blue-50 p-3 rounded-lg text-center border border-blue-200 col-span-2">
                        <div className="text-xs text-blue-600">Total Score</div>
                        <div className="text-lg font-bold text-blue-700">{unitScore.percentage}%</div>
                        <div className="text-xs text-gray-400">({unitScore.totalScore} / {unitScore.maxScore})</div>
                      </div>
                    </div>
                  </div>
                </div>
            ...
            {formData.units && formData.units.length > 0 && (
              <section className="bg-white shadow-sm rounded-lg border border-gray-200 overflow-hidden mb-6">
                <div className="bg-purple-700 px-6 py-3">
                  <h2 className="text-lg font-semibold text-white">📊 Total Score Semua Unit</h2>
                </div>
                <div className="p-6">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    <div className="bg-gray-50 p-4 rounded-lg text-center border border-gray-200">
                      <div className="text-sm text-gray-500">Total Unit</div>
                      <div className="text-2xl font-bold text-gray-700">{formData.units.length}</div>
                    </div>
                    <div className="bg-blue-50 p-4 rounded-lg text-center border border-blue-200">
                      <div className="text-sm text-blue-600">Total Item</div>
                      <div className="text-2xl font-bold text-blue-700">{totalStats.totalItems}</div>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg text-center border border-green-200">
                      <div className="text-sm text-green-600">Total Score</div>
                      <div className="text-2xl font-bold text-green-700">{totalStats.percentage}%</div>
                      <div className="text-xs text-gray-400">({totalStats.totalScore} / {totalStats.maxScore})</div>
                    </div>
                    <div className="bg-purple-50 p-4 rounded-lg text-center border border-purple-200">
                      <div className="text-sm text-purple-600">Status</div>
                      <div className={`text-lg font-bold ${totalStats.percentage >= 80 ? 'text-green-600' : totalStats.percentage >= 60 ? 'text-yellow-600' : 'text-red-600'}`}>
                        {totalStats.percentage >= 80 ? '✅ Baik' : totalStats.percentage >= 60 ? '⚠️ Perlu Perhatian' : '❌ Perlu Perbaikan'}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="bg-blue-50 p-4 rounded-lg text-center border border-blue-200">
                    ...
                    <div className="text-2xl font-bold text-blue-700">
                      {(() => {
                        ...
                        formData.inspectionData.forEach((item: any) => {
                          const originalItem = inspectionItems.find(i => i.id === item.id);
                          if (originalItem) {
...
function Dashboard({ role, onLogout }: { 
  ...
  const handleSaveReport = (data: any) => {
    ...
    if (data.status === 'approved') {
      ...
      alert('📝 REVISION! Dikirim ke Maintenance untuk perbaikan.');
    } else if (data.status === 'qc_approved') {
      alert('📤 Data dikirim ke Maintenance!');
    } else if (data.status === 'maintenance_done') {
      alert('🔧 Perbaikan selesai! QC sekarang bisa verifikasi.');
    } else {
  ...
  const handleExportExcel = () => {
    ...
    allData.push({
      'Unit': `Jumlah Unit: ${selectedReport.jumlahUnit}`,
      'No Unit': `Teknisi: ${selectedReport.teknisiName || '-'}`,
      'No': '',
      'Item Pemeliharaan': '',
      'Status': '',
      'Temuan QC': '',
    ...
    selectedReport.units.forEach((unit: UnitData, idx: number) => {
      ...
      allData.push({
        ...
        'No Unit': '',
        'No': '',
        'Item Pemeliharaan': `SCORE UNIT ${idx + 1}`,
        'Status': '',
        'Temuan QC': `${unitScore.percentage}%`,
        'Catatan Perbaikan': `(${unitScore.totalScore}/${unitScore.maxScore})`,
        'Score': ''
      });
      allData.push({});
    });
    ...
    allData.push({
      'Unit': 'Total Unit',
      'No Unit': selectedReport.jumlahUnit,
      'No': '',
      'Item Pemeliharaan': 'Total Score',
      'Status': '',
      'Temuan QC': `${totalStats.percentage}%`,
      'Catatan Perbaikan': `(${totalStats.totalScore}/${totalStats.maxScore})`,
      'Score': ''
    });

    const ws = XLSX.utils.json_to_sheet(allData);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Data Pemeliharaan');
  ...
  const calculateUnitScoreForExport = (unitData: UnitData) => {
    ...
    return { totalScore, maxScore, percentage: Math.round(percentage) };
  };

  const calculateTotalScoreForExport = (report: Report) => {
    if (!report.units || report.units.length === 0) {
    ...
    report.units.forEach((unit: UnitData) => {
      const score = calculateUnitScoreForExport(unit);
      totalScore += score.totalScore;
      maxScore += score.maxScore;
    });
                ...
                {isQC && (
                  <>
                    <button 
                      ...
                      className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg flex items-center gap-2 shadow-md"
                    >
              ...
              <div className="flex items-center gap-2">
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm transition ${
                    sortOrder === 'terbaru' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  📅 Terbaru
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-md text-sm transition ${
                    sortOrder === 'terlama' ? 'bg-blue-600 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'
                  }`}
                >
                  📅 Terlama
                </button>
                ...
                <div>
                  <h2 className="font-semibold text-gray-700">
                    {isQC 
                      ...
                      : isMaintenance 
                        ? `🔧 Tugas Maintenance`
                        : '📋 Data Pemeriksaan (Sales)'
                    }
                  </h2>
                  <span className="text-sm text-gray-500">
                    {filteredReports.length} laporan
                  </span>
                  {isMaintenance && <span className="text-xs text-blue-500 ml-2">(Semua tugas dari QC)</span>}
                  {isSales && <span className="text-xs text-orange-500 ml-2">(Read-Only)</span>}
                </div>
              </div>
                ...
                if (filteredReports.length === 0) {
                  return (
                    <div className="p-8 text-center text-gray-500">
                      {isQC 
                        ? <p>Belum ada laporan. Klik tombol <strong>"New Pemeriksaan"</strong> atau <strong>"New Pemeliharaan"</strong> untuk mulai.</p>
                        : isMaintenance 
                          ? <p>Belum ada tugas dari QC.</p>
                          : <p>Belum ada laporan Pemeriksaan dari QC.</p>
                      }
                    </div>
                  );
                }
                    ...
                    <table className="w-full" style={{ fontSize: '14px' }}>
                      ...
                      <tbody>
                        {filteredReports.map((report) => {
                          ...
                          const unitDisplay = report.reportType === 'pemeliharaan' && report.jumlahUnit > 1 
                            ? `${report.jumlahUnit} Unit` 
                            : firstUnit?.unitNumber || '-';
                          const projectDisplay = report.reportType === 'pemeliharaan' && report.jumlahUnit > 1
                            ? `${report.jumlahUnit} Proyek`
                            : firstUnit?.projectCode || '-';
                          const customerDisplay = report.reportType === 'pemeliharaan' && report.jumlahUnit > 1
                            ? `${report.jumlahUnit} Pelanggan`
                            : firstUnit?.customerName || '-';

                          return (
                            <tr key={report.id} className="border-t border-gray-100 hover:bg-gray-50">
                              <td className="px-4 py-2 text-gray-500 whitespace-nowrap" style={{ fontSize: '13px' }}>
                              ...
                              <td className="px-4 py-2">
                                ...
                                {report.reportType === 'pemeliharaan' && report.jumlahUnit > 1 && (
                                  <span className="ml-1 text-xs text-gray-500">({report.jumlahUnit} unit)</span>
                                )}
                              </td>
                              ...
                              <td className="px-4 py-2">
                                <button 
                                  ...
                                  className="text-blue-600 hover:text-blue-800 font-medium" style={{ fontSize: '13px' }}
                                >
                                  👁️ Lihat
                                </button>
                                {isQC && report.status !== 'approved' && (
                                  <button 
                                    ...
                                    className="text-red-500 hover:text-red-700 font-medium ml-2" style={{ fontSize: '13px' }}
                                  >
                                    🗑️
                                  </button>
                                )}
